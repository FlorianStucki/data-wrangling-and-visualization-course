...
count(data_amp_raw)
count(data_demographics_raw)
count(data_selfreport_raw)
...
# use a function without the pipe
pipe3 <- mutate(data_amp_raw, mean_cor = mean(correct))
...
pipe4 <- data_amp_raw%>%
  mutate(mean_cor = mean(correct))

# check they produce identical results
identical(pipe3,pipe4)
...
pipe5<-  clean_names(data_amp_raw)
pipe5 <-  rename(pipe5,
   unique_id = subject,
   test_case = blockcode,
   test_type = trialcode)
pipe5<-  group_by(pipe5,test_type)
pipe5<-  filter(pipe5,grepl("practice", test_type, ignore.case = TRUE))
pipe5<-  mutate(pipe5,sum_real = n())
pipe5<-  group_by(pipe5,unique_id)
pipe5<-  summarize(pipe5,n())
...
# use a series of functions with the pipe
 ...
 group_by(test_type)%>%
   filter(grepl("practice", test_type, ignore.case = TRUE))%>%
   mutate(sum_real = n())%>%
...
print(names(data_demographics_clean_names))

# print all column names as a vector
data_demographics_clean_names_names <- names(data_demographics_clean_names)
print(data_demographics_clean_names_names)
...
data_amp_clean <- data_amp_renamed%>%
  select(unique_id,block,trial_type)

data_selfreport_clean <- data_selfreport_renamed%>%
  select(unique_id,item)

data_demographics_clean <- data_demographics_renamed%>%
  select(unique_id,item)
...
data_amp_clean <- read_csv(file = "../data/raw/data_amp_raw.csv")%>%
  clean_names()%>%
  rename(unique_id = subject,
         block = blockcode,
         trial_type = trialcode)%>%
...
data_selfreport_clean <- read_csv(file = "../data/raw/data_selfreport_raw.csv")%>%
  clean_names()%>%
  rename(unique_id = subject,
         item = trialcode)%>%
  select(unique_id,item, response)

data_demographics_clean <- read_csv(file = "../data/raw/data_demographics_raw.csv")%>%
  clean_names()%>%
  rename(unique_id = subject,
         item = trialcode)%>%
  select(unique_id,item,response)
...
data_demographics_clean %>%
  count(unique_id)
data_demographics_clean%>% count(item)
data_demographics_clean%>% count(response)
...
equivalence <- data_amp_clean %>%
  filter(correct ==1)

# example using negation
negation <- data_amp_clean %>%
  filter(trial_type != "age")

# example using membership
membership <- data_amp_clean %>%
  filter(trial_type %in% c("gender","age"))
...
MC1 <- data_amp_clean %>%
  filter(trial_type %in% c("gender") & trial_type %in% c("age"))
MC2 <- data_amp_clean %>%
  filter(trial_type %in% c("gender") | trial_type %in% c("age"))
...
```{r}
data_amp_clean2<- data_amp_clean%>%
  filter(block != "practice")

data_selfreport_clean2 <- data_selfreport_clean %>%
...
e1 <- data_amp_raw %>%
  mutate(Juni = date %in% c("23.06.22"))

# mutating the contents of existing variables
e2 <- data_amp_clean %>%
  mutate(latency = latency*0.6)
...
data_demographics_gender_tidy_1 <- data_demographics_clean %>%
  # filter only the gender item, not age
  filter(item == "gender") %>%
...
# clean up the gender variable
data_demographics_gender_tidy_2 <- data_demographics_clean %>%
  # filter only the gender item, not age
  filter(item == "gender") %>%
  # change the name of the response variable to what it now represents: gender
  rename(gender = response) %>%
...
data_amp_test_trials_with_fast_trials <- data_amp_clean %>%
...
data_demographics_age_tidy <- data_demographics_clean %>%
  filter(item == "age") %>%
...
# what values are present?
data_selfreport_clean2 %>%
  count(response)

# what type of data is the response column?
class(data_selfreport_clean2$response)

# remove non Likert values
data_selfreport_tidy <- data_selfreport_clean2 %>%
...
Adresses directli the problem, but code is not useable for new data.

```{r}

# write examples here

data_selfreport_tidy3 <- data_selfreport_clean2 %>%
...
data_amp_reverse <- data_amp_clean2 %>%
  mutate(
  case_when(
    correct == 1 ~ 0,
    correct == 0~ 1,
...
```{r}
data_selfreport_raw <- read.csv(file = "../data/raw/data_selfreport_raw.csv") |>  
  select(unique_id = subject, item = trialcode, response)

data_selfreport_mean_sd <- data_selfreport_raw |> 
  group_by(unique_id) %>% 
  count() %>%
  filter(n == 4) |> 
  rename(number = "n")%>%
  left_join(data_selfreport_raw, by = "unique_id") |>
  select(-number) |> 
   filter(item != "instructions") |> 
