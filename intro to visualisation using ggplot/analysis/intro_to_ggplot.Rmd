...
```{r}
amp_mean <- mean(data_processed$amp_score)
ggplot(data_processed,aes(amp_score))+
  geom_histogram(binwidth =0.1 )+
  scale_x_continuous(breaks = seq(0,1,0.1),
                     name ="AMP Score")+
  scale_y_continuous(breaks = seq(0,40,5),
                     name ="Frequency")+
  geom_vline(xintercept = amp_mean, linetype="dotet")
...
```{r}
ggplot(data_after_exclusions,aes(amp_score))+
...
summary_selfreport <- data_after_exclusions |> 
  group_by(gender) |> 
  summarise(selfreport_mean=mean(mean_self_report), selfreport_se = std.error(mean_self_report))

ggplot(summary_selfreport, aes(gender,selfreport_mean,fill = gender))+
  geom_col() +
    geom_errorbar(aes(ymin = selfreport_mean - selfreport_se, 
